...
There are a lot of existing tutorials for learning the command line, so we're not going to try to recreate that here. 
...
4. Use `pwd` `cd` and `ls` to navigate through the TesseractOCR folder.
